...
async function subscribe(plan, button) {
  ...
  // Сервер эту проверку тоже делает (иначе оплата и не пройдёт), но
  // проверить на клиенте -- значит не гонять человека через оплату
  // впустую и сказать понятно сразу, а не показывать ответ сервера.
  if (!me.registered) {
    $('msg').innerHTML =
